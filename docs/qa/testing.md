...
??? question "What's the difference between a unit test, an instrumented test, and a UI test on Android?"
    ...
    ```kotlin
    @Test fun addition() { assertEquals(4, 2 + 2) }                         // unit — plain JVM

    @Test fun clickButton() { onView(withId(R.id.button)).perform(click()) } // instrumented/UI
    ```
...
??? question "What is the Testing Pyramid and how does it apply to Android?"
    ...
    ```kotlin
    // many:  @Test fun computesTotal()        <- unit tests
    // some:  @Test fun repositorySyncsToDb()  <- integration tests
    // few:   @Test fun checkoutFlowEndToEnd() <- slow, full UI tests
    ```
...
??? question "How would you unit test a ViewModel with a coroutine-based Repository dependency?"
    ...
    fun `loads user`() = runTest {
        ...
        vm.load("1")
        assertEquals(UiState.Content("Ada"), vm.state.value)
    }
    ```
...
??? question "What is `TestDispatcher`/`runTest` for, and why can't you just use `runBlocking` reliably in coroutine tests?"
    ...
    ```kotlin
    @Test
    fun test() = runTest {  // virtual time — this resolves instantly, not after 10 real seconds
        delay(10_000)
        assertTrue(true)
    }
    ```
...
??? question "What's the difference between mocking and faking?"
    ...
    ```kotlin
    val mock = mockk<UserRepository> { every { getUser(any()) } returns User("test") } // configured stand-in

    class FakeUserRepository : UserRepository { // a real, simplified implementation
...
??? question "When would you prefer a fake over a mock?"
    ...
    class InMemoryUserRepository : UserRepository {
        ...
        override suspend fun getUser(id: String) = users[id] // behaves like the real thing
    }
    ```
...
??? question "What is Espresso used for, and how does it achieve synchronization with the UI thread automatically?"
    ...
    ```kotlin
    onView(withId(R.id.button)).perform(click()) // Espresso waits for the UI to go idle first
    ```
...
??? question "What is an `IdlingResource` and when do you need to register a custom one?"
    ...
    class MyIdlingResource : IdlingResource {
        override fun isIdleNow(): Boolean = !isBusy // tells Espresso when custom work is done
    }
    ```
...
??? question "What's the difference between `onNodeWithText` and `onAllNodesWithText` in Compose testing?"
    ...
    ```kotlin
    composeRule.onNodeWithText("Submit")      // fails if 0 or 2+ match
    composeRule.onAllNodesWithText("Item")[2] // index into one of several matches
    ```
...
??? question "What is Robolectric and when would you choose it over a real device/emulator test?"
    ...
    class MyTest {
        @Test fun test() { assertNotNull(ApplicationProvider.getApplicationContext<Context>()) }
    }
    ```
...
??? question "What are common causes of flaky Android tests?"
    ...
    ```kotlin
    Thread.sleep(2000) // classic flakiness — use an IdlingResource or awaitItem() instead
    ```
...
??? question "What's the difference between test doubles at the Repository layer vs mocking the network client (e.g., OkHttp) directly?"
    ...
    ```kotlin
    class FakeUserRepository : UserRepository { /* fast — isolates the ViewModel test */ }
    // vs
    val server = MockWebServer().apply { enqueue(MockResponse().setBody(json)) } // exercises real parsing
    ```
...
??? question "What's the difference between testing a `suspend fun` Repository method versus one returning `Flow`?"
    ...
    ```kotlin
    @Test fun suspendTest() = runTest { assertEquals(user, repo.getUser("1")) }

    @Test fun flowTest() = runTest {
        repo.observeUser("1").test { assertEquals(user, awaitItem()) }
    }
    ```
...
??? question "Why is testing `equals()`/`hashCode()` correctness important for data classes used as Compose keys or Map keys?"
    ...
    ```kotlin
    data class Item(val id: String, var cache: Bitmap? = null) // a mutable field here breaks Map lookups
    ```
...
??? question "What is contract testing / consumer-driven contract testing and where might it fit in a mobile + backend team setup?"
    ...
    ```json
    { "GET /user/1": { "status": 200, "body": { "name": "string" } } }
    ```
...
??? question "What's the difference between a snapshot test and an assertion-based UI test?"
    ...
    ```kotlin
    paparazzi.snapshot { MyScreen() }                 // pixel-level comparison
    composeRule.onNodeWithText("Ada").assertExists()  // checks one semantic property
    ```
...
??? question "What is Espresso-Intents, and what does it let you verify that plain Espresso view assertions can't?"
    ...
    intended(hasAction(Intent.ACTION_SEND)) // verified without launching the real target app
    ```
...
??? question "What's the difference between `ActivityScenario` and the older `ActivityTestRule`?"
    ...
    scenario.moveToState(Lifecycle.State.CREATED) // fine-grained lifecycle control
    ```
...
??? question "What is `FragmentScenario` used for, and why is it valuable for testing Fragments in isolation?"
    ...
    ```kotlin
    val scenario = launchFragmentInContainer<MyFragment>() // isolated host, no real nav graph needed
    ```
...
??? question "What's the difference between Google's Truth assertion library and plain JUnit assertions?"
    ...
    ```kotlin
    assertThat(list).containsExactly("a", "b").inOrder() // Truth — fluent, typed
    assertEquals(listOf("a", "b"), list)                  // JUnit — generic
    ```
...
??? question "What's the difference between a 'relaxed' mock and a strict mock in MockK?"
    ...
    ```kotlin
    val relaxed = mockk<Api>(relaxed = true) // unstubbed calls return defaults instead of throwing
    val strict = mockk<Api>()                 // unstubbed calls throw
    ```
...
??? question "What's the difference between a 'hermetic' test and a test that depends on real network/external services?"
    ...
    ```kotlin
    class MyTest {
        private val repo = FakeUserRepository() // no real network/DB — fast, offline, deterministic
    }
    ```
...
??? question "Why would you write a custom Robolectric 'Shadow' class, and what problem does it solve?"
    ...
    ```kotlin
    @Implements(SomeFrameworkClass::class)
    class ShadowSomeFrameworkClass {
        @Implementation fun someMethod() = "overridden behavior for this test"
    }
    ```
